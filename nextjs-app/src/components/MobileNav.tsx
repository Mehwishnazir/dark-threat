...
function NavSection({ label, links, onClose }: SectionProps) {
  ...
  const hasActive = links.some((item) => isActivePath(pathname, item.to, item.to === "/industries"));
  ...
  return (
    <div className="border-b border-border/40 last:border-0">
      <button
        ...
        aria-expanded={open}
        className={`w-full flex items-center justify-between px-4 py-3.5 text-sm font-semibold uppercase tracking-[0.12em] transition-colors ${
          hasActive ? "text-primary" : "text-muted-foreground hover:text-primary"
        }`}
      >
        <span>{label}</span>
        <ChevronDown
      ...
      >
        <div className="pb-3 px-2">
          {links.map((item) => {
            const active = isActivePath(pathname, item.to, item.to === "/industries");
            return (
              <Link
                key={item.to}
                href={item.to}
    ...
    <div className="lg:hidden">
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-border shrink-0">
          <Link href="/" onClick={close} aria-label="DarkThreat Home" className="flex items-center gap-2">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain">
          <div className="px-3 pt-3 pb-1">
            {topLinks.map((link) => {
              const active = isActivePath(pathname, link.to, Boolean(link.end));
              return (
                <Link
                  key={link.to}
                  href={link.to}
