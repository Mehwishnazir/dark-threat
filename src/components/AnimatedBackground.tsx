...
export default function AnimatedBackground() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      streams.forEach((stream) => {
        ...
        const trailX = stream.x - (dx / Math.max(distance, 1)) * trailLength;
        const trailY = stream.y - (dy / Math.max(distance, 1)) * trailLength;
  ...
  return (
    <canvas
      ...
      style={{
        background:
          "radial-gradient(circle at 20% 50%, hsla(0, 100%, 50%, 0.03) 0%, transparent 50%), radial-gradient(circle at 80% 20%, hsla(0, 100%, 50%, 0.03) 0%, transparent 50%)",
        zIndex: 1,
      }}
    />
  );
