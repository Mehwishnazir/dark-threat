...
import {
  Bitcoin, Shield, Lock, AlertTriangle, Database, CheckCircle2,
  ArrowRight, Key, Eye, Wallet,
} from 'lucide-react';
...
export default function CryptoFintechIndustry() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submitLeadForm({
        formType: 'Industry Inquiry (Crypto & Fintech)',
        name: form.name,
        email: form.email,
        company: form.company,
        message: form.message,
      });
      toast({ title: 'Request Received', description: 'A crypto & fintech threat specialist will contact you within 24 hours.' });
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Dark Web Monitoring for Crypto &amp; Fintech | DarkThreat</title>
        <meta name="description" content="Monitor wallet address leaks, exchange credential theft, and DeFi protocol attack signals. FinCEN, MiCA, and ISO 27001 aligned dark web monitoring for crypto and fintech." />
        <link rel="canonical" href="https://darkthreat.ai/industries/crypto-fintech" />
      ...
      <section className="relative min-h-[62vh] flex flex-col items-center justify-center overflow-hidden pt-32 pb-16 hero-bg-layered">
        ...
        <div className="relative z-10 text-center max-w-5xl mx-auto px-6">
          ...
          <div className="mt-8 flex flex-wrap gap-4 justify-center">
            <Button className="hero-button" onClick={() => document.getElementById('cf-inquiry-form')?.scrollIntoView({ behavior: 'smooth' })}>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id="cf-name" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} required placeholder="Jane Doe" className="mt-1 bg-background/50" />
                </div>
                <div>
                  <Label htmlFor="cf-email">Business Email *</Label>
                  <Input id="cf-email" type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} required placeholder="jane@exchange.com" className="mt-1 bg-background/50" />
                </div>
              </div>
              <div>
                <Label htmlFor="cf-company">Platform / Company Name</Label>
                <Input id="cf-company" value={form.company} onChange={e => setForm(p => ({ ...p, company: e.target.value }))} placeholder="Your exchange, wallet, or fintech" className="mt-1 bg-background/50" />
              </div>
              <div>
                <Label htmlFor="cf-message">Primary Concern</Label>
                <Textarea id="cf-message" value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} rows={4} placeholder="E.g. wallet key exposure, exchange credential leaks, API key dumps..." className="mt-1 bg-background/50" />
              </div>
