...
import {
  Landmark, Shield, Lock, AlertTriangle, Database, CheckCircle2,
  ArrowRight, FileWarning, Key, Eye, Globe, Cpu
} from 'lucide-react';
...
export default function GovernmentIndustry() {
  const [form, setForm] = useState({ name: '', email: '', agency: '', region: '', message: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submitLeadForm({
        formType: 'Industry Inquiry (Government)',
        name: form.name,
        email: form.email,
        company: form.agency,
        interest: form.region,
        message: form.message,
      });
      toast({ title: 'Assessment Request Received', description: 'A government cyber threat specialist will contact you within 24 hours.' });
      setForm({ name: '', email: '', agency: '', region: '', message: '' });
    } catch (err) {
      toast({
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Government Dark Web Monitoring — Protecting Public Infrastructure | DarkThreat</title>
        <meta name="description" content="Secure government agencies, citizen data, and critical infrastructure from nation-state actors and ransomware. FISMA & NIST SP 800-53 aligned." />
        <link rel="canonical" href="https://darkthreat.ai/industries/government" />
      ...
      <section className="relative min-h-[62vh] flex flex-col items-center justify-center overflow-hidden pt-32 pb-16 hero-bg-layered">
        ...
        <div className="relative z-10 text-center max-w-5xl mx-auto px-6">
          ...
          <div className="mt-8 flex flex-wrap gap-4 justify-center">
            <Button className="hero-button" onClick={() => document.getElementById('gov-inquiry-form')?.scrollIntoView({ behavior: 'smooth' })}>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id="gov-name" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} required placeholder="Major John Doe" className="mt-1 bg-background/50" />
                </div>
                <div>
                  <Label htmlFor="gov-email">Official Email *</Label>
                  <Input id="gov-email" type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} required placeholder="john.doe@agency.gov" className="mt-1 bg-background/50" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="gov-agency">Agency / Organization</Label>
                  <Input id="gov-agency" value={form.agency} onChange={e => setForm(p => ({ ...p, agency: e.target.value }))} placeholder="Department of Defense" className="mt-1 bg-background/50" />
                </div>
                <div>
                  <Label htmlFor="gov-region">Jurisdiction / Region</Label>
                  <Input id="gov-region" value={form.region} onChange={e => setForm(p => ({ ...p, region: e.target.value }))} placeholder="Federal / State / Local" className="mt-1 bg-background/50" />
                </div>
              </div>
              <div>
                <Label htmlFor="gov-message">Primary Mission Focus</Label>
                <Textarea id="gov-message" value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} rows={4} placeholder="E.g., protecting voter files, defense supplier assessments, nation-state activity indicators..." className="mt-1 bg-background/50" />
              </div>
              <Button type="submit" disabled={isSubmitting} className="hero-button w-full">
                {isSubmitting ? 'Requesting Scan...' : 'Request Agency Scan'}
              </Button>
            </form>
          </div>
        </div>
