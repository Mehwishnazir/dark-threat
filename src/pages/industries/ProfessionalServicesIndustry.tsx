...
import {
  Briefcase, Shield, Lock, AlertTriangle, Database, CheckCircle2,
  ArrowRight, Key, Eye,
} from 'lucide-react';
...
export default function ProfessionalServicesIndustry() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submitLeadForm({
        formType: 'Industry Inquiry (Professional Services)',
        name: form.name,
        email: form.email,
        company: form.firm,
        message: form.message,
      });
      toast({ title: 'Request Received', description: 'A professional services threat specialist will contact you within 24 hours.' });
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Dark Web Monitoring for Professional Services | DarkThreat</title>
        <meta name="description" content="Protect confidential client information, partner credentials, and firm intellectual property. SOC 2, ISO 27001, and GDPR aligned dark web monitoring for consulting, accounting, and advisory firms." />
        <link rel="canonical" href="https://darkthreat.ai/industries/professional-services" />
      ...
      <section className="relative min-h-[62vh] flex flex-col items-center justify-center overflow-hidden pt-32 pb-16 hero-bg-layered">
        ...
        <div className="relative z-10 text-center max-w-5xl mx-auto px-6">
          ...
          <div className="mt-8 flex flex-wrap gap-4 justify-center">
            <Button className="hero-button" onClick={() => document.getElementById('ps-inquiry-form')?.scrollIntoView({ behavior: 'smooth' })}>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              ...
              <Link to="/industries/legal" className="text-primary hover:underline">Legal industry page</Link>.
            </p>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id="ps-name" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} required placeholder="Jane Doe" className="mt-1 bg-background/50" />
                </div>
                <div>
                  <Label htmlFor="ps-email">Work Email *</Label>
                  <Input id="ps-email" type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} required placeholder="jane@firm.com" className="mt-1 bg-background/50" />
                </div>
              </div>
              <div>
                <Label htmlFor="ps-firm">Firm Name</Label>
                <Input id="ps-firm" value={form.firm} onChange={e => setForm(p => ({ ...p, firm: e.target.value }))} placeholder="Your consulting, accounting, or advisory firm" className="mt-1 bg-background/50" />
              </div>
              <div>
                <Label htmlFor="ps-message">Primary Concern</Label>
                <Textarea id="ps-message" value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} rows={4} placeholder="E.g. partner credential leaks, client data exposure, vendor risks..." className="mt-1 bg-background/50" />
              </div>
