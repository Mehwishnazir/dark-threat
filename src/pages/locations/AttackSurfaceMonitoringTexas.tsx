...
export default function AttackSurfaceMonitoringTexas() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submitLeadForm({
        formType: 'Location Inquiry (Attack Surface Monitoring - Texas)',
        name: form.name,
        email: form.email,
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Attack Surface Monitoring in Texas | Enterprise Cybersecurity by DarkThreat</title>
        <meta name="description" content="Protect your Texas organization with advanced Attack Surface Monitoring. Detect and remediate threats before they become data breaches. Book a demo today." />
        <link rel="canonical" href="https://darkthreat.ai/attack-surface-monitoring/texas" />
          ...
          <div className="rounded-3xl border border-border bg-card/40 p-8 backdrop-blur-md shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <Button type="submit" disabled={isSubmitting} className="hero-button w-full">
                {isSubmitting ? 'Requesting Scan...' : 'Request Free Scan'}
              </Button>
            </form>
          </div>
        </div>
